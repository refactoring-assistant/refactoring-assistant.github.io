class Shipment {
    ...
    public double calculateDistance() {
        double dLat = degToRad(destCity.getLatitude() - originCity.getLatitude());
        double dLon = degToRad(destCity.getLongitude() - originCity.getLongitude());
        double a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
                Math.cos(degToRad(originCity.getLatitude())) * Math.cos(degToRad(destCity.getLatitude())) *
                        Math.sin(dLon / 2) * Math.sin(dLon / 2);
        double c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
        double d = this.EARTHRADIUS * c;
        return d;
    }
