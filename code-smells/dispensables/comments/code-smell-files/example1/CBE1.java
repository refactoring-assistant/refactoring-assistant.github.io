 ...
 */
class ImageModel {
    private final HashMap<String, ImageBuffer> loadedImages; // A map to store loaded images by name
    ...
    // Helper method to clamp values between 0 and 255 (valid RGB values)
    ...
    public ImageBuffer alterBrightness(ImageBuffer image, float brightness) {
        ...
        // Create a new Buffer to store the modified image
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                // Adjust brightness by adding to each color component
                red = clamp((int) (red + brightness));
                green = clamp((int) (green + brightness));
                blue = clamp((int) (blue + brightness));

                // Recombine the adjusted RGB components into a single int
                long newRGB = (red << 16) | (green << 8) | blue;

                // Set the new RGB value in the brightened image
